...
Here we initialize our :class:`~ndf_parse.Mod` object. It's nothing but a
...
python representation (based on :mod:`~ndf_parse.model`) and returns a
:class:`~ndf_parse.model.List` instance to the user to work with. Since
:class:`~ndf_parse.Mod` is implemented as a context manager, as soon as
``with`` statement's scope is closed (i.e. when all of the operations defined in
...
All classes that are subclasses of :class:`~ndf_parse.model.DeclarationsList`
are iterable as well as have length (``len(source)``). This means that you can
iterate through :class:`~ndf_parse.model.List`,
:class:`~ndf_parse.model.Object` , :class:`~ndf_parse.model.Template` and
:class:`~ndf_parse.model.Map`. Stray pairs (ones that are not inside of a Map)
are defined as a :class:`tuple`. Everything else is essentially a string
literal. This also means that List, Object, Template and Map all have a
:attr:`~ndf_parse.model.DeclarationsList.parent` attribute while others don't.
It's worth noting that :class:`~ndf_parse.model.Template` also has a
:attr:`~ndf_parse.model.Template.params` attribute to store it's generic
parameters.

Each returned ``obj_row`` is a subclass of a
:class:`~ndf_parse.model.DeclListRow` Here is a correspondence table of all
list-likes and their rows:

+-------------------------+------------------------------------+-------------------------------------+
|          Notes          |            Parent Class            |              Row Class              |
+=========================+====================================+=====================================+
| root/lists/vector types | :class:`~ndf_parse.model.List`     | :class:`~ndf_parse.model.ListRow`   |
+-------------------------+------------------------------------+-------------------------------------+
| template parameters     | :class:`~ndf_parse.model.Params`   | :class:`~ndf_parse.model.ParamRow`  |
+-------------------------+------------------------------------+-------------------------------------+
| templates               | :class:`~ndf_parse.model.Template` | :class:`~ndf_parse.model.MemberRow` |
+-------------------------+------------------------------------+-------------------------------------+
| objects                 | :class:`~ndf_parse.model.Object`   | :class:`~ndf_parse.model.MemberRow` |
+-------------------------+------------------------------------+-------------------------------------+
| maps                    | :class:`~ndf_parse.model.Map`      | :class:`~ndf_parse.model.MapRow`    |
+-------------------------+------------------------------------+-------------------------------------+
...
1.  :class:`~ndf_parse.model.List`, :class:`~ndf_parse.model.Object` and
    :class:`~ndf_parse.model.Template` all store their type inside of their own
    :attr:`~ndf_parse.model.Object.type` attribute instead of row's one. This
    issue is covered in detail in :ref:`typing ambiguity <typing-ambiguity>`
    section.
...
There is a :func:`~ndf_parse.walk` function that is designed to help you find
...
:func:`~ndf_parse.expression`. It takes a string with an ndf statement and
returns a :class:`dict` with the desired object and additional arguments (if
...
- :func:`ndf_parse.printer.string`, returns ndf code as a string instead of
  dumping it to stdout.
- :func:`ndf_parse.printer.format`, wtires ndf code out to an IO handle.
  :func:`~ndf_parse.printer.print` used above is actually a wrapper around
  this function.
